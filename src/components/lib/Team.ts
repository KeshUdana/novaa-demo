export const Team = [
  {
    UserName: "Keshawa Udana",
    role: ["Supreme Leader", "Frontend Developer"],
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ad nostrum nesciunt laborum nulla quisquam ullam reprehenderit molestias amet officiis optio dolor, quasi, consectetur itaque quod tempore et veniam ab quam?",
    path: "/team/keshawa-udana",
    tech: ["React", "Next.js", "Vite", "Python"],
    imgURL:
      "https://images.unsplash.com/photo-1634926878768-2a5b3c42f139?q=80&w=1956&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    class: "left",
  },
  {
    UserName: "Achintha Ekanayake",
    role: ["ML Engineer", "AI Specialist"],
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ad nostrum nesciunt laborum nulla quisquam ullam reprehenderit molestias amet officiis optio dolor, quasi, consectetur itaque quod tempore et veniam ab quam?",
    path: "/team/achintha-ekanayake",
    tech: ["Anaconda", "Java", "Python"],
    imgURL:
      "https://images.unsplash.com/photo-1593085512500-5d55148d6f0d?q=80&w=1780&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    class: "right",
  },
  {
    UserName: "Anuja Rahul",
    role: ["Frontend Developer", "Backend Developer"],
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ad nostrum nesciunt laborum nulla quisquam ullam reprehenderit molestias amet officiis optio dolor, quasi, consectetur itaque quod tempore et veniam ab quam?",
    path: "/team/anuja-rahul",
    tech: ["React", "Next.js", "Python", "GSAP", "SASS", "TailwindCSS"],
    imgURL:
      "https://images.unsplash.com/photo-1634896941598-b6b500a502a7?q=80&w=1956&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    class: "left",
  },
  {
    UserName: "Buddila Ruwanpathirana",
    role: ["ML Engineer", "AI Specialist"],
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ad nostrum nesciunt laborum nulla quisquam ullam reprehenderit molestias amet officiis optio dolor, quasi, consectetur itaque quod tempore et veniam ab quam?",
    path: "/team/buddila-ruwanpathirana",
    tech: ["Anaconda", "Java", "Python"],
    imgURL:
      "https://images.unsplash.com/photo-1635003913011-95971abba560?q=80&w=1956&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    class: "right",
  },
  {
    UserName: "Chalaka Buthsara",
    role: ["UI/UX", "Frontend Developer"],
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ad nostrum nesciunt laborum nulla quisquam ullam reprehenderit molestias amet officiis optio dolor, quasi, consectetur itaque quod tempore et veniam ab quam?",
    path: "/team/chalaka-buthsara",
    tech: ["Figma", "Anaconda", "React", "Next.js", "Python"],
    imgURL:
      "https://images.unsplash.com/photo-1634193295627-1cdddf751ebf?q=80&w=1887&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
    class: "left",
  },
];
